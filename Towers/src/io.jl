...
function readInputFile(inputFile::String)
    ...
    grid = Array{Int}(undef, n, n)
    haut = map(x->parse(Int64,x),split(data[1],","))
    bas = map(x->parse(Int64,x),split(data[2],","))
    gauche = map(x->parse(Int64,x),split(data[3],","))
    droite = map(x->parse(Int64,x),split(data[4],","))
...
"""
function displayGrid(haut,bas,gauche,droite)
...
"""
function displaySolution(x::Array{VariableRef,3}, haut, bas, gauche, droite)
	...
	displaySolution(t,haut,bas,gauche,droite)
end
"""
...
"""
function displaySolution(t, haut, bas, gauche, droite)
...
function saveInstance(haut, bas, gauche, droite, outputFile::String)
            ...
            for resultFile in filter(x->occursin(".txt", x), readdir(path))
                ...
                if isOptimal
                    ...
                    if solveTime > maxSolveTime
                        ...
                        # println("file", resultFile)
                        # println("maxSolveTime", maxSolveTime)
                    end 
                end 
            end 
        end
